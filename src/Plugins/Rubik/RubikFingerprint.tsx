...
export function RubikFingerprint({ filter }: { filter: number[] }) {
    const data = useAppSelector((state) => state.data.rows);
    const vectors = filter.map((index) => data[index])

    return vectors.length > 0 ? <Center><RubikFingerprintIntern width={200} height={270} vectors={vectors} /></Center> : null;
}

export class RubikFingerprintIntern extends React.Component<RubikFingerprintProps> {
