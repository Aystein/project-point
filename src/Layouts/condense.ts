...
export function runCondenseLayout(n: number, area: IRectangle) {
    return runLayout({ n, area }, new Worker(new URL('../Workers/condense.worker.ts', import.meta.url)))
}

export function runGroupLayout(X, area: IRectangle, feature: string) {
    return runLayout({ X, area, feature }, new Worker(new URL('../Workers/group.worker.ts', import.meta.url)))
}