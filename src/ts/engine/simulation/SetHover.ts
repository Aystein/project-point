...
export class SetHover {
    ...
    private static readonly setHoverStructType: WebGPU.Types.StructType = new WebGPU.Types.StructType("HoverValue", [
        ...
        this.pipeline = device.createComputePipeline({
            ...
            compute: {
                module: WebGPU.ShaderModule.create(device, {
                    code: ShaderSources.Engine.Simulation.SetHover,
                    structs: [data.particlesBufferData.particlesStructType, this.uniforms, SetHover.setHoverStructType],
                }),
                entryPoint: "main",
                constants: {
                    workgroupSize: SetHover.WORKGROUP_SIZE,
                },
            },
        });
    ...
    private applyReset(data: Data): ResetResult {
        const workgroupsCount = Math.ceil(data.particlesBufferData.particlesCount / SetHover.WORKGROUP_SIZE);
        ...
        const bindgroup = this.device.createBindGroup({
            ...
            entries: [
                {
                    binding: 0,
                    resource: data.hoverBuffer.bindingResource,
                },
                {
                    binding: 1,
