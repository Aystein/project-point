...
function Statistics() {
  ...
  return <div>{rows.length} rows and {columns.length} columns</div>
}
...
function DatasetList() {
  ...
  return <Flex direction={"column"} p={'1rem'}>
    {datasets.map((name) => {
      return <NavLink key={name} onClick={() => handleLoad(name)} label={name} active />
    })}
  </Flex>
}
