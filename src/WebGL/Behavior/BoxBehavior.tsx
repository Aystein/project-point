...
import { Rectangle } from "../Container/Rectangle";
...
export function BoxBehavior() {
  ...
  useMouseEvent(
    ...
    (event) => {
      setRect((rect) => {
        return new Rectangle(rect.x, rect.y, event.offsetX - rect.x, event.offsetY - rect.y);
      })
      return true;
    },
    [rect, setRect]
  ...
  return <div style={{ position: 'absolute', width: '100%', height: '100%', pointerEvents: 'none' }}>
    {rect ? <div style={{
      position: 'absolute',
      left: rect.x,
      top: rect.y,
